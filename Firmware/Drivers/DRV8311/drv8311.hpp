#ifndef __DRV8311_HPP
#define __DRV8311_HPP
...
public:
    typedef enum : uint32_t {
        ...
        FaultType_GVDD_UV  = (1 << 9),  //!< DRV8311 Vdd Under Voltage fault
        FaultType_FAULT    = (1 << 10),

        // Status Register 2
        FaultType_GVDD_OV  = (1 << 23)  //!< DRV8311 Vdd Over Voltage fault
    } FaultType_e;

    Drv8311(Stm32SpiArbiter* spi_arbiter, Stm32Gpio ncs_gpio,
    ...
    /**
     * @brief Returns true if and only if the DRV8311 chip is in an initialized
...
private:
    enum CtrlMode_e {
        DRV8311_CtrlMode_Read = 1 << 15,   //!< Read Mode
        DRV8311_CtrlMode_Write = 0 << 15   //!< Write Mode
    };
    ...
    /** @brief Reads data from a DRV8311 register */
    ...
    RegisterFile regs_; //!< Current configuration. If is_ready_ is
                        ...
                        //!< with the actual file on the DRV8311 chip.
